...
static int help_command(int argc, char **argv)
{
    ...
    bool oneCommandMode = help_args.command->count >= 1;
    ...
    SLIST_FOREACH(it, &s_cmd_list, next) {
        ...
        if ( oneCommandMode ) {
            if ( strcmp( it->command, help_args.command->sval[0] ) != 0 ) {
                continue;
            }
        } else {
            char buf[ 50 ] = {};
            strncpy( buf, it->help, sizeof( buf ) - 1 );
            char * pFind = strchr( buf, '\n' );
            if ( pFind != NULL ) {
                *pFind = '\0';
            }
            printf("  %s:  %s%s\n", it->command, buf,
                   strlen( it->help ) > sizeof( buf ) ? "..." : "" );
            continue;
        }
...
{
    help_args.command = arg_str0( NULL, NULL, "<command>", "command name");
